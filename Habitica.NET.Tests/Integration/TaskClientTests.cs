...
namespace Habitica.NET.Tests.Integration
{
    public class TaskClientTests
    {
        ...
        [Fact]
        public async Task CreateChallengeTaskAsync_Normal_ExecutesRequestProperly()
        {
            (var client, var handler) = InstrumentedTaskClient();
            ...
            _ = await client.CreateChallengeTaskAsync(challengeId, tasks);
            var request = handler.LastRequest;

            Assert.Equal($"/api/v3/tasks/challenge/{challengeId}", request.RequestUri.AbsolutePath);
            Assert.Equal("", request.RequestUri.Query);
        ...
        public async Task DeleteTaskChecklistItemAsync_Normal_ExecutesRequestProperly()
        {
            ...
            await client.DeleteTaskChecklistItemAsync(taskId, itemId);
            var request = handler.LastRequest;

            Assert.Equal($"/api/v3/tasks/{taskId}/checklist/{itemId}", request.RequestUri.AbsolutePath);
            Assert.Equal("", request.RequestUri.Query);
            Assert.Null(request.Content);
        }

        [Fact]
        public async Task DeleteTaskTagAsync_Normal_ExecutesRequestProperly()
        {
            (var client, var handler) = InstrumentedTaskClient();

            string taskId = "taskId";
            Guid itemId = Guid.NewGuid();
            await client.DeleteTaskTagAsync(taskId, itemId);
            var request = handler.LastRequest;

            Assert.Equal($"/api/v3/tasks/{taskId}/tags/{itemId}", request.RequestUri.AbsolutePath);
            Assert.Equal("", request.RequestUri.Query);
            Assert.Null(request.Content);
        }

        [Fact]
        public async Task DeleteTaskAsync_Normal_ExecutesRequestProperly()
        {
            (var client, var handler) = InstrumentedTaskClient();
            ...
            Assert.Equal($"/api/v3/tasks/{taskId}", request.RequestUri.AbsolutePath);
            Assert.Equal("", request.RequestUri.Query);
            Assert.Null(request.Content);
        }

        [Fact]
        public async Task ClearCompletedTodosAsync_Normal_ExecutesRequestProperly()
        {
            (var client, var handler) = InstrumentedTaskClient();

            await client.ClearCompletedTodosAsync();
            var request = handler.LastRequest;

            Assert.Equal("/api/v3/tasks/clearCompletedTodos", request.RequestUri.AbsolutePath);
            Assert.Equal("", request.RequestUri.Query);
            Assert.Null(request.Content);
        }
